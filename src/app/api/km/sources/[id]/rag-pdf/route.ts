...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const buffer = await generateRagPdf(source.ragContent, source.title);
  const safeTitle = source.title.replace(/[\\/:*?"<>|]/g, "_") || "rag-content";
  const encodedTitle = encodeURIComponent(`${safeTitle}.pdf`);

  return new Response(new Uint8Array(buffer), {
    headers: {
      "Content-Type": "application/pdf",
      // 檔名可能含中文，Content-Disposition 的 filename 只能是 ASCII，中文檔名要用 RFC 5987 的 filename* 表示。
      "Content-Disposition": `attachment; filename="rag-content.pdf"; filename*=UTF-8''${encodedTitle}`,
    },
  });
}
